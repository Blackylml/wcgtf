...
import { submitMatchBet } from "./actions";
...
export function MatchCard({ match }: { match: Match }) {
  ...
  const [currentBet, setCurrentBet] = useState<MatchPick | null>(match.userBet);
  ...
  const dateStr = date.toLocaleDateString("es-MX", {
    weekday: "short", day: "2-digit", month: "short", timeZone: "America/Monterrey",
  });
  const timeStr = date.toLocaleTimeString("es-MX", {
    hour: "2-digit", minute: "2-digit", hour12: false, timeZone: "America/Monterrey",
  });

  async function handlePick(pick: MatchPick) {
    setLoading(true);
    setError("");
    const result = await submitMatchBet(match.id, pick);
    setLoading(false);
    if (result?.error) setError(result.error);
    else if (result?.redirectUrl) window.location.href = result.redirectUrl;
    else setCurrentBet(pick);
  }

  const betLabelMap: Record<MatchPick, string> = { HOME: homeName, DRAW: "Empate", AWAY: awayName };

  return (
    <div
      className={`rounded-2xl border border-white/[0.08] bg-white/[0.025] p-4 h-full transition-opacity ${
        !match.isOpen && !currentBet ? "opacity-50" : ""
      }`}
    >
      {/* Header */}
      ...
      {currentBet ? (
        <div className="flex items-center justify-center gap-1.5 bg-green-400/[0.1] border border-green-400/25 rounded-xl py-2.5">
          ...
          <span className="text-green-300 text-xs font-semibold truncate">{betLabelMap[currentBet]}</span>
        </div>
      ) : match.isOpen ? (
        ...
        <div className="flex items-center justify-center gap-1.5 text-xs text-slate-600 py-2">
      ...
      {match.price > 0 && !currentBet && match.isOpen && (
